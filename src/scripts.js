...
function showOrderInfo(order) {
    ...
    // Настройка кнопки "Готово" в зависимости от статуса заказа
    ...
    if (markCompletedBtn) {
        ...
        if (order.status === 'waiting') {
            newMarkCompletedBtn.innerText = 'Готово';
            newMarkCompletedBtn.style.display = 'block';
            newMarkCompletedBtn.onclick = function() {
                markOrderCompleted(order.id, order.status);
            };
        } else if (order.status === 'completed') {
            newMarkCompletedBtn.innerText = 'Удалить';
            newMarkCompletedBtn.style.display = 'block';
            newMarkCompletedBtn.onclick = function() {
    ...
    if (confirm('Вы уверены, что хотите удалить этот заказ?')) {
        db.ref('orders/' + id).remove()
            .then(function() {
                ...
                closeOrderInfoModal(); // Закрываем модальное окно после удаления
            })
...
// Функция для обновления инвентаря при оформлении заказа
function updateInventory(sizeWidth, sizeLength, quantity) {
    // Поиск соответствующего инвентаря
    db.ref('inventory').orderByChild('sizeWidth').equalTo(sizeWidth).once('value', function(snapshot) {
        var inventoryFound = false;
        snapshot.forEach(function(childSnapshot) {
            var inventory = childSnapshot.val();
            var inventoryId = childSnapshot.key;
            if (inventory.sizeLength === sizeLength) {
                inventoryFound = true;
                // Используем транзакцию для безопасного обновления
                db.ref('inventory/' + inventoryId + '/quantity').transaction(function(currentQuantity) {
                    return (currentQuantity || 0) - quantity;
                }, function(error, committed, snapshot) {
                    if (error) {
                        console.error("Ошибка при обновлении инвентаря: ", error);
                        alert("Произошла ошибка при обновлении инвентаря. Пожалуйста, попробуйте еще раз.");
                    } else if (!committed) {
                        console.log("Транзакция не была завершена.");
                    } else {
                        console.log("Инвентарь успешно обновлен.");
                        // Обновление статуса инвентаря
                        var updatedQuantity = snapshot.val();
                        var newStatus = updatedQuantity < LOW_STOCK_THRESHOLD ? 'У вас низкие запасы' : 'Достаточно';
                        db.ref('inventory/' + inventoryId + '/status').set(newStatus);
                    }
                });
            }
        });
        ...
        if (!inventoryFound) {
            // Если инвентарь не найден, создаем новый с отрицательным количеством
            var newInventoryRef = db.ref('inventory').push();
            newInventoryRef.set({
                sizeWidth: sizeWidth,
                sizeLength: sizeLength,
                quantity: -quantity,
                status: 'Низкие запасы'
            }, function(error) {
                if (error) {
                    console.error("Ошибка при добавлении нового инвентаря: ", error);
                    alert("Произошла ошибка при добавлении нового инвентаря. Пожалуйста, попробуйте еще раз.");
                } else {
                    console.log("Новый инвентарь успешно добавлен.");
                }
            });
        }
    });
    ...
    sizes.forEach(function(sizeObj) {
        ...
        db.ref('orders').push(order, function(error) {
            if (error) {
                ...
                // После успешного добавления заказа, обновляем инвентарь
                updateInventory(order.size, order.length, order.quantity);
            }
        });
    });
...
function createOrderDiv(order) {
    ...
    var daysLeft = Math.ceil(timeDiff / (1000 * 60 * 60 * 24)); // Количество оставшихся дней
...
// Функция для изменения статуса заказа на "Готово" или удаления заказа
function markOrderCompleted(orderId, currentStatus) {
